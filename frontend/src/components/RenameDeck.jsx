...
const RenameDeck = ({ id, name, close, refresh }) => {
    ...
    return (
        <>
            ...
            <form className="fixed top-1/2 left-1/2 -translate-1/2 w-1/2 p-8 bg-gray-50 backdrop-blur-xl border border-gray-200 rounded-xl shadow-lg" onSubmit={handleRenameDeck}>
                <IoCloseOutline className='absolute right-5.5 top-5.5 text-3xl cursor-pointer text-gray-600 hover:text-black' onClick={() => close()}/>
                <h1 className="text-2xl font-bold text-gray-900 mb-4">Renomear <span className='text-primary'>{name}</span></h1>
                <input
                    type="text"
                    id='deckName'
                    name='deckName'
